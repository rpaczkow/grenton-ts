...
interface IDimm {
    ...
    setDimmingEdge: (type: DimmingEdgeType) => void
...
class Dimm implements IDimm {
    ...
    setDimmingEdge(type: DimmingEdgeType): void {
        this.raw.set(PropertyType.DimmingEdge, type);
    }
    ...
    get dimmingEdge(): DimmingEdgeType {
        return this.raw.get(PropertyType.DimmingEdge);
    }

    set dimmingEdge(value: DimmingEdgeType) {
        this.raw.set(PropertyType.DimmingEdge, value);
    }
...
class DimmRemote implements IDimm {
    ...
    setDimmingEdge(type: DimmingEdgeType): void {
        const cmd: string | null = rawExecutionBuilderFactory(this.objectName)
            .set()
            .addParameter(PropertyType.DimmingEdge)
            .addParameter(type)
            .build();
        this.gate.runScript(cmd!);
    }
...
export {
    Dimm, DimmRaw, DimmRemote, StatisticStateType, DimmingEdgeType
}
