// Created from: src/interfaces/module_infibity_thermostat_radiator_valve_ff.xml, object name="ZWAVE_WAKEUP"
...
class ZwaveWakeup implements IZwaveWakeup {
    ...
    setInterval(value: number): void { this.raw.set(PropertyType.Interval, value); }
}
...
class ZwaveWakeupRemote implements IZwaveWakeup {
    ...
    setInterval(value: number): void {
        const cmd = rawExecutionBuilderFactory(this.objectName).set().addParameter(PropertyType.Interval).addParameter(value).build();
        this.gate.runScript(cmd!);
    }
}
