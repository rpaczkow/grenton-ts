// Created from: src/interfaces/module_2_0_MULTISENSOR_fv01_02.xml, object name="AIR_CO2_SENSOR"
...
interface IAirCo2Sensor {
    ...
    /** Wartość estymowana CO2 w zakresie od 400 do 60000 ppm */
    readonly value: number
    /** Wielkość histerezy (dokładność 1 ppm) określająca czułość, przy której następuje wygenerowanie zdarzeń: OnValueChange, OnValueLower, OnValueRise */
    threshold: number
    ...
    /**
     * Wymuszenie kalibracji czujnika CO2 (czas kalibracji do 12 h)
     * @param {number} state
     */
    recalibration: (state?: number) => void
}

class AirCo2Sensor implements IAirCo2Sensor {
    ...
    constructor(private raw: AirCo2SensorRaw) {
     ...
     * @param {number} state
     */
    recalibration(state: number = 0): void {
        this.raw.execute(MethodType.Recalibration, state);
    }
}

class AirCo2SensorRemote implements IAirCo2Sensor {
    ...
    recalibration(state: number = 0): void {
        const cmd: string | null = rawExecutionBuilderFactory(this.objectName)
            .execute()
            .addParameter(MethodType.Recalibration)
            .addParameter(state)
            .build();
        this.gate.runScript(cmd!);
    }
...
export { AirCo2Sensor, AirCo2SensorRaw, AirCo2SensorRemote, StatisticStateType }
