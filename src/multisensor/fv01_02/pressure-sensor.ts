// Created from: src/interfaces/module_2_0_MULTISENSOR_fv01_02.xml, object name="PRESSURE_SENSOR"
...
declare class PressureSensorRaw {
...
interface IPressureSensor {
    ...
    /** Wartość ciśnienia atmosferycznego od 300 do 1100 hPa */
    readonly value: number
    /** Wielkość histerezy (dokładność 0.1 hPa) określająca czułość, przy której następuje wygenerowanie zdarzeń: OnValueChange, OnValueLower, OnValueRise */
    threshold: number
    ...
    maxValue: number
    /** Wysokość punktu pomiarowego w metrach nad poziomem morza */
    altitude: number
    /** Włącza raportowanie pomiaru do modułu statystyk */
    statisticState: StatisticStateType
}

class PressureSensor implements IPressureSensor {
    ...
    constructor(private raw: PressureSensorRaw) {
    ...
    get altitude(): number {
        return this.raw.get(PropertyType.Altitude);
    }
    set altitude(value: number) {
        this.raw.set(PropertyType.Altitude, value);
    }
...
class PressureSensorRemote implements IPressureSensor {
    ...
    get altitude(): number {
        const cmd: string | null = rawExecutionBuilderFactory(this.objectName)
            .get()
            .addParameter(PropertyType.Altitude)
            .build();
        return this.gate.runScript(cmd!);
    }
    set altitude(value: number) {
...
export { PressureSensor, PressureSensorRaw, PressureSensorRemote, StatisticStateType }
