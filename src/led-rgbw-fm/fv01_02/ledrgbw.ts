// Created from: src/interfaces/module_2_0_LED_RGBW_FM_fv01_02.xml, object name="LEDRGBW"
...
enum PropertyType {
    ...
    Load = 14,
    WhiteValue = 15,
    RedChannelDistributedLogicGroup = 16,
    GreenChannelDistributedLogicGroup = 17,
    BlueChannelDistributedLogicGroup = 18,
    WhiteChannelDistributedLogicGroup = 19
}
...
interface ILedrgbw {
    ...
    /** Grupa Distributed Logic - grupa broadcastowa kanału czerwonego dla rozproszonej logiki */
    redChannelDistributedLogicGroup: number
    /** Grupa Distributed Logic - grupa broadcastowa kanału zielonego dla rozproszonej logiki */
    greenChannelDistributedLogicGroup: number
    /** Grupa Distributed Logic - grupa broadcastowa kanału niebieskiego dla rozproszonej logiki */
    blueChannelDistributedLogicGroup: number
    /** Grupa Distributed Logic - grupa broadcastowa kanału białego dla rozproszonej logiki */
    whiteChannelDistributedLogicGroup: number
...
class Ledrgbw implements ILedrgbw {
    ...
    constructor(private raw: LedrgbwRaw) {
    ...
    get redChannelDistributedLogicGroup(): number { return this.raw.get(PropertyType.RedChannelDistributedLogicGroup); }
    set redChannelDistributedLogicGroup(val: number) { this.raw.set(PropertyType.RedChannelDistributedLogicGroup, val); }
    get greenChannelDistributedLogicGroup(): number { return this.raw.get(PropertyType.GreenChannelDistributedLogicGroup); }
    set greenChannelDistributedLogicGroup(val: number) { this.raw.set(PropertyType.GreenChannelDistributedLogicGroup, val); }
    get blueChannelDistributedLogicGroup(): number { return this.raw.get(PropertyType.BlueChannelDistributedLogicGroup); }
    set blueChannelDistributedLogicGroup(val: number) { this.raw.set(PropertyType.BlueChannelDistributedLogicGroup, val); }
    get whiteChannelDistributedLogicGroup(): number { return this.raw.get(PropertyType.WhiteChannelDistributedLogicGroup); }
    set whiteChannelDistributedLogicGroup(val: number) { this.raw.set(PropertyType.WhiteChannelDistributedLogicGroup, val); }
...
class LedrgbwRemote implements ILedrgbw {
    ...
    get redChannelDistributedLogicGroup(): number {
        const cmd = rawExecutionBuilderFactory(this.objectName).get().addParameter(PropertyType.RedChannelDistributedLogicGroup).build();
        return this.gate.runScript(cmd!);
    }
    set redChannelDistributedLogicGroup(val: number) {
        const cmd = rawExecutionBuilderFactory(this.objectName).set().addParameter(PropertyType.RedChannelDistributedLogicGroup).addParameter(val).build();
        this.gate.runScript(cmd!);
    }
    get greenChannelDistributedLogicGroup(): number {
        const cmd = rawExecutionBuilderFactory(this.objectName).get().addParameter(PropertyType.GreenChannelDistributedLogicGroup).build();
        return this.gate.runScript(cmd!);
    }
    set greenChannelDistributedLogicGroup(val: number) {
        const cmd = rawExecutionBuilderFactory(this.objectName).set().addParameter(PropertyType.GreenChannelDistributedLogicGroup).addParameter(val).build();
        this.gate.runScript(cmd!);
    }
    get blueChannelDistributedLogicGroup(): number {
        const cmd = rawExecutionBuilderFactory(this.objectName).get().addParameter(PropertyType.BlueChannelDistributedLogicGroup).build();
        return this.gate.runScript(cmd!);
    }
    set blueChannelDistributedLogicGroup(val: number) {
        const cmd = rawExecutionBuilderFactory(this.objectName).set().addParameter(PropertyType.BlueChannelDistributedLogicGroup).addParameter(val).build();
        this.gate.runScript(cmd!);
    }
    get whiteChannelDistributedLogicGroup(): number {
        const cmd = rawExecutionBuilderFactory(this.objectName).get().addParameter(PropertyType.WhiteChannelDistributedLogicGroup).build();
        return this.gate.runScript(cmd!);
    }
    set whiteChannelDistributedLogicGroup(val: number) {
        const cmd = rawExecutionBuilderFactory(this.objectName).set().addParameter(PropertyType.WhiteChannelDistributedLogicGroup).addParameter(val).build();
        this.gate.runScript(cmd!);
    }
...
export { Ledrgbw, LedrgbwRaw, LedrgbwRemote, StatisticStateType }
