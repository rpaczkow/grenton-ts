// Created from: src/interfaces/module_2_0_DALI_GEAR_DT8_fv03_08.xml, object name="DALI_GEAR_DT8"
...
interface IDaliGearDt8 {
    ...
    /** Ustawiana temperatura barwowa na podstawie wywołanej metody SetColourTemperature. Cecha nie pobiera rzeczywistego stanu temperatury barwowej oprawy */
...
class DaliGearDt8 implements IDaliGearDt8 {
    ...
    get colourTemperature(): number { return this.raw.get(PropertyType.ColourTemperature); }
    set colourTemperature(val: number) { this.raw.set(PropertyType.ColourTemperature, val); }
...
class DaliGearDt8Remote implements IDaliGearDt8 {
    ...
    get colourTemperature(): number {
        const cmd = rawExecutionBuilderFactory(this.objectName).get().addParameter(PropertyType.ColourTemperature).build();
        return this.gate.runScript(cmd!);
    }
    set colourTemperature(val: number) {
        const cmd = rawExecutionBuilderFactory(this.objectName).set().addParameter(PropertyType.ColourTemperature).addParameter(val).build();
        this.gate.runScript(cmd!);
    }
