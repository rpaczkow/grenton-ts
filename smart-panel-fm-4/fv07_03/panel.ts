// Created from: packages/grenton-api/interfaces/module_SMART_PANEL_FM_4_fv07_03.xml, object name="PANEL" version="07.03"
...
interface IPanel {
    ...
    draw: () => void
    /**
     * Zablokowanie działania przycisków, gestów oraz odświeżania ekranu na czas czyszczenia panelu (czas trwania blokady wyrażony w sekundach)
     * @param {number} duration
     */
    startCleaningMode: (duration: number) => void
...
class PanelRemote implements IPanel {
    ...
    startCleaningMode(duration: number): void {
        const cmd: string | null = rawExecutionBuilderFactory(this.objectName)
            .execute()
            .addParameter(MethodType.StartCleaningMode)
            .addParameter(duration)
            .build();
        this.gate.runScript(cmd!);
    }
...
class Panel implements IPanel {
     ...
     */
    startCleaningMode(duration: number): void {
        this.raw.execute(MethodType.StartCleaningMode, duration);
    }
    /** Nazwa pliku BMP z ikoną dla gestu Góra (bez rozszerzenia) */
    get gestureIconUp(): string {
